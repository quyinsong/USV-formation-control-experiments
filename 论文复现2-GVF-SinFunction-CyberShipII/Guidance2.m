function [nu_c,sigma_hato] = Guidance2( hi, ai0, zi_bar, nu_bar, vs, R_psi, theta, ri, p0, ts)
...
% nu_c = hi\(-Ki*zi_bar/sqrt(norm(zi_bar)^2+emi^2)-sigma_hat);


C = diag([1/sqrt(norm(zi_bar)^2+0.01),1/sqrt(zi_bar(2)^2+0.01)]);
D = [tanh(zi_bar(1)/0.1),tanh(zi_bar(2)/0.2)]';
