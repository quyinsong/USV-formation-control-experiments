function rc = GVF2( x,y,psi,beta,rc )
%GVF 此处显示有关此函数的摘要
%   此处显示详细说明
...
Ud = 0.3;
% boundary 1
R1 = 34; % 最外层边界，进入该边界后碰撞行为和跟踪行为同时存在
d1 = -2; % 内层边界，进入该边界后只存在避碰行为
phir1 = ((y-70)/20)^2-x+R1;
no1 = [-1 2*((y-70)/20)*(1/20)]'; taur1 = E*no1;
H1 = [0 0; 0 0];
kod = -1;
...
if phir1 <= d1
    f1 = 0; f2 = 1;
elseif phir1 < 0
    l1 = 10; l2 = 1;  % l1 越大则避障优先级越高 l2 越大则路径跟踪优先级越高
...
% boundary 2
R2 = 26; % 最外层边界，进入该边界后碰撞行为和跟踪行为同时存在
d2 = -2; % 内层边界，进入该边界后只存在避碰行为
phir2 = x+((y-70)/20)^2-R2;
no2 = [1 2*((y-70)/20)*(1/20)]'; taur2 = E*no1;
H2 = [0 0; 0 1/200];
...
if phir2 <= d2
    f1 = 0; f2 = 1;
elseif phir2 < 0
    l1 = 1; l2 = 1;  % l1 越大则避障优先级越高 l2 越大则路径跟踪优先级越高
