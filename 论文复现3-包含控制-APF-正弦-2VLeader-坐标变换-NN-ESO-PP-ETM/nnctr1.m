function [tau_c,e_bar,f_hat] = nnctr1( nu_tf, nu_c, tau, tauc, Gamma, q, vartheta,zeta, ts )
...
tu1 = 5*su-ucf_dot+fu_hat+Tu*alpha_u+q(1)*vartheta(1);
tr1 = 5*sr-rcf_dot+fr_hat+Tr*alpha_r+q(2)*vartheta(2);

% tu1 = 5*su-ucf_dot+Tu*alpha_u+fu_hat;
% tr1 = 5*sr-rcf_dot+Tr*alpha_r+fr_hat;

% tuc = -(1+0.01)*(su*tu1^2*M(1,1))/sqrt(su^2*tu1^2+0.001)-fu_hat*M(1,1);
% trc = -(1+0.01)*(sr*tr1^2*M(2,2))/sqrt(sr^2*tr1^2+0.001)-fr_hat*M(2,2);
...
% 权值更新
W_u_dot = 1*(e_bar(1)*Phi_u-0.5*W_u);
W_u = W_u_dot*ts+W_u;
